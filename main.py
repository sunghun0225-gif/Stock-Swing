...
def get_stock_news(ticker, market="US"):
    ...
    try:
        ...
        # 한국 뉴스 검색 최적화
        hl = 'en-US' if market == 'US' else 'ko-KR'
        gl = 'US' if market == 'US' else 'KR'
        ceid = 'US:en' if market == 'US' else 'KR:ko'
...
# ==========================================
# 탭 2: 한국 종목 스캐너 (이름 검색 지원)
# ==========================================
with tab2:
    st.subheader("🇰🇷 한국 시장 종목 분석 (이름 검색)")
    
    col_k1, col_k2 = st.columns([3, 1])
    with col_k1:
        search_name = st.text_input("한국 종목명 입력", placeholder="예: 삼성전자, 에코프로", key="kr_name_in")
    
    if search_name:
        try:
            # yfinance 검색 엔진 가동
            search = yf.Search(search_name, max_results=5)
            # 한국 시장(KS, KQ) 결과만 필터링
            options = [f"{q['longname']} ({q['symbol']})" for q in search.quotes if ".KS" in q['symbol'] or ".KQ" in q['symbol']]
            
            if options:
                selected_match = st.selectbox("검색 결과에서 선택하세요:", options)
                ticker_to_add = selected_match.split("(")[1].replace(")", "")
                
                if st.button("선택한 종목 추가"):
                    if ticker_to_add not in st.session_state["my_tickers_kr"]:
                        st.session_state["my_tickers_kr"].append(ticker_to_add)
                        st.success(f"{selected_match} 추가 완료!")
                        st.rerun()
            else:
                st.warning("일치하는 한국 종목을 찾지 못했습니다. 정확한 명칭을 입력해 주세요.")
        except:
    ...
    if st.button("🚀 한국 종목 스캔 시작", use_container_width=True):
        for t in sel_kr:
            ...
            h = s.history(period="6mo")
            if not h.empty:
                st.success(f"**[{t}]** 현재가: {int(h['Close'].iloc[-1]):,} 원")
                with st.expander("정보 확인"):
                    code_only = t.split(".")[0]
                    st.markdown(f"🏛️ [네이버 증권 공시](https://finance.naver.com/item/news.naver?code={code_only})")
                    for n in get_stock_news(t, "KR"): st.markdown(f"- [{n['title']}]({n['link']})")
            st.write("---")

# ==========================================
# 탭 3: 정밀 분할 매수 (지점별 가격 계산)
# ==========================================
with tab3:
    ...
    st.markdown("#### 📉 각 회차별 하락 목표치 (%)")
    st.caption("1차 진입가 대비 누적 하락률을 입력하세요.")
    r_cols = st.columns(4)
