...
with tab3:
    ...
    if num_rounds > 1:
        ...
        drop_rates = []
        remaining = num_rounds - 1
        idx = 0
        while idx < remaining:
            batch = min(4, remaining - idx)
    ...
    if total_budget > 0 and start_price > 0:
        rates = [0.0] + drop_rates
        total_weight = sum(2 ** i for i in range(num_rounds))
        base_unit = total_budget / total_weight

        data = []
        cum_amount = 0.0
        cum_shares = 0.0
        prev_avg = start_price
        ...
        component_height = 160 + len(data) * 46 + 80
  ...
  <script>
    async function saveImg() {{
      ...
      try {{
        ...
        // iframe 환경에 맞춰 즉시 다운로드 방식으로만 처리
        ...
        status.textContent = '✅ 저장이 완료되었습니다. 다운로드 폴더를 확인하세요.';
      }} catch(e) {{
        status.textContent = '❌ 오류: ' + e.message;
      }}
      
      btn.disabled = false;
      btn.textContent = '📷 이미지로 다시 저장';
    }}
  </script>
</body>
</html>
